...
async fn handle_conns_loop(mut incoming: Incoming, client_map: ClientMap, key_map: KeyMap, auth: settings::AuthRules) {
    ...
    while let Some(conn) = incoming.next().await {
        ...
        // tokio::spawn(ses);
        handles.push(ChildTask::from(tokio::spawn(ses)));
        // No need for this, when this function returns the whole damn server will crash anyways!
    }
}
